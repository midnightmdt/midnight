...
#if defined(NDEBUG)
# error "Midnight cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Midnight Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
...

    {
        ...
        // Check against previous transactions
        // This is done last to help prevent CPU exhaustion denial-of-service attacks.
        if (!tx.ConnectInputs(txdb, mapInputs, mapUnused, CDiskTxPos(1,1,1), pindexBest, false, false, STANDARD_SCRIPT_VERIFY_FLAGS, true))
        {
...
int64_t GetProofOfWorkReward(int64_t nFees, int nHeight)
{
    ...
    else if(nHeight <= 1072800)
    {
	nSubsidy = 0.5 * COIN;
    }
    else if(nHeight <= 1598400)
    {
	nSubsidy = 0.25 * COIN;
    }
    else
    ...
    // override, every 720 blocks the reward is 100 coins
    if(nHeight % 720 == 0)
    {
	nSubsidy = 100 * COIN;
    }
...
bool CTransaction::ConnectInputs(CTxDB& txdb, MapPrevTx inputs, map<uint256, CTxIndex>& mapTestPool, const CDiskTxPos& posThisTx,
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags, bool fMasternodeCheck)
{
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    {
        LogPrintf("ProcessMessage: pfrom->nVersion == 0, must have version message before anything else\n");
        ...
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                {
                    ...
                    hashRand = HashBmw256(BEGIN(hashRand), END(hashRand));
                    ...
                    BOOST_FOREACH(CNode* pnode, vNodes)
                    {
                        ...
                        hashKey = HashBmw256(BEGIN(hashKey), END(hashKey));
    ...
    else if (strCommand == "getaddr") //((strCommand == "getaddr") && (pfrom->fInbound))
    {
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        uint256 hash = HashBmw256(vRecv.begin(), vRecv.begin() + nMessageSize);
