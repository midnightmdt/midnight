...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("midnight"))
        return false;
...
{
    // Convert midnight:// to midnight:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("midnight://"))
    {
        uri.replace(0, 12, "midnight:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Midnight.lnk";
}
...
{
    return GetAutostartDir() / "midnight.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Midnight-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  midnight-qt [" + tr("command-line options") + "]                     " + "\n";
...
void SetBlackThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(41,44,48); }"
                      ...
                      "QToolButton:checked { color: rgb(255,255,255); border: none; border-left-color: rgb(0,27,226); border-left-style: solid; border-left-width: 6px; }"
                      "QProgressBar   { color: rgb(0,27,226); border-color: rgb(255,255,255); border-width: 3px; border-style: solid; }"
