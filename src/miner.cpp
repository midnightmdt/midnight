...
{
    LogPrintf("MidnightMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("midnight-miner");
	...
	CBlock *pblock = pblocktemplate.get();
        ...
        LogPrintf("Running MidnightMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("MidnightMiner terminated\n");
        throw;
    }
}
