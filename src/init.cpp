...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: midnight.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: midnightd.pid)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Midnight is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Midnight is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Midnight version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
 ...
 CMasterNode::minProtoVersion = GetArg("-masternodeminprotocol", MIN_MN_PROTO_VERSION);
    ...
    if (fDaemon)
        fprintf(stdout, "Midnight server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Midnight") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Midnight to complete") << "\n";
...
fMasterNode = GetBoolArg("-masternode", false);
    ...
    nAnonymizeMidnightAmount = GetArg("-anonymizebananabitsamount", 0);
    if(nAnonymizeMidnightAmount > 999999) nAnonymizeMidnightAmount = 999999;
    if(nAnonymizeMidnightAmount < 2) nAnonymizeMidnightAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeMidnightAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize Midnight Amount %d\n", nAnonymizeMidnightAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1MDT+1000 == (.1MDT+100)*10
       10MDT+10000 == (1MDT+1000)*10
    */
