    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Midnight and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002MDT per transaction on average.
            */
            if(r <= 20)
            {
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeMidnightAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fMidnightSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f MDT CENT - sessionTotalValue %d\n", fMidnightSubmitted, sessionTotalValue);
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("midnight-darksend");
	...
	if(c % 600 == 0) { // every 10 minutes, broadcast our list of masternodes
            BOOST_FOREACH(CMasterNode mn, vecMasternodes) {
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeMidnightAmount = (rand() % nLeftToAnon)+3;
                } else {
